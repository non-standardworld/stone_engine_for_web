...
| 段階 | 期間 | 目標 | 成果物 |
|------|------|------|--------|
| **MVP** | 3週間 | 基本的な日本語レンダリング | 動くプロトタイプ |
| **検証** | 1週間 | 技術的実現可能性の確認 | 評価レポート |
| **Phase 1-2** | 4週間 | 高度組版機能 | 縦書き・禁則・約物 |
| **Phase 3** | 2週間 | UI/UX | エディター機能 |
| **Phase 4** | 2週間 | 最適化 | 商用レベル |
...
| 機能 | MVP (Web標準) | 本格版 (HarfBuzz) |
|------|---------------|-------------------|
| **横書き日本語** | ✅ 十分 | ✅ 完璧 |
| **縦書き日本語** | ⚠️ 基本的 | ✅ 完璧 |
| **OpenType GSUB** | ⚠️ CSS font-feature-settings | ✅ HarfBuzz処理 |
| **複雑な約物処理** | ⚠️ 手動実装 | ✅ HarfBuzz処理 |
| **多言語対応** | ❌ 困難 | ✅ 容易 |
| **パフォーマンス** | 🟡 中 | 🟢 高 |

**判断ポイント**: MVP検証後、品質が不十分ならHarfBuzz導入
...
|------|---------|------|
| **表示品質** | 文字が正しく表示される | ✅/❌ |
| **フォント選択** | Script別にフォントが切り替わる | ✅/❌ |
| **改行処理** | 自動折り返しが自然 | ✅/❌ |
| **パフォーマンス** | 1000文字 < 100ms | ✅/❌ |
...
✅ 品質・パフォーマンス共に合格
  → Path A: Web標準技術で継続（Phase 1へ）

⚠️ 品質はOKだがパフォーマンスに課題
  → Path B: 最適化後、Phase 1へ

❌ 品質に問題あり（特にGSUB置換）
  → Path C: HarfBuzz導入を検討
```

**判断基準:**
- 縦書きグリフ置換が必要 → HarfBuzz導入
- 複雑な約物処理が必要 → HarfBuzz導入
- 多言語対応が必要 → HarfBuzz導入
- 上記以外 → Web標準で継続
...
| # | 成果物 | 説明 | 納期 |
|---|--------|------|------|
| 1 | **MVP** | 横書き日本語表示ライブラリ | Week 3 |
| 2 | **評価レポート** | MVP検証結果と次ステップ提案 | Week 4 |
| 3 | **Phase 1完成版** | 縦書き・禁則・約物対応 | Week 8 |
| 4 | **Phase 3完成版** | 編集機能付きライブラリ | Week 10 |
| 5 | **最終版** | 最適化済み商用レベル | Week 12 |
| 6 | **APIドキュメント** | TypeDoc生成 | Week 12 |
| 7 | **デモサイト** | GitHub Pages | Week 12 |

### 9.2 マイルストーン

```
M0: プロジェクトセットアップ (Day 1-2)
  ✅ Viteプロジェクト作成
  ✅ TypeScript設定
  ✅ ディレクトリ構成

M1: MVPデータモデル完成 (Week 1)
  ✅ 型定義
  ✅ Context実装
  ✅ Unicode分類
